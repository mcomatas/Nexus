...
export default function Page({ params }) {
  ...
  const developersFiltered = game.involved_companies?.filter(
    (company) => company.developer,
  );
  const publishersFiltered = game.involved_companies?.filter(
    ...
    <div>
      ...
      {game.artworks && (
        <div className="flex flex-col h-130 items-center relative">
          <Image
            src={"https:" + game.artworks[0].url.replace("t_thumb", "t_1080p")}
            alt={String(slug)}
            layout="fill"
            style={{ objectFit: "cover", objectPosition: "center 20%" }}
            quality={100}
          />
          <div className="absolute inset-0 bg-gradient-to-t from-background-end from-10% to-transparent to-50%" />
        </div>
      )}
      <div
        className={`flex flex-row max-w-7/10 min-h-[500px] mx-auto ${game.artworks && "-mt-25"} p-2 relative`}
      >
        <div>
          <ImageModal
            thmb={
            ...
            full={
              game.cover
                ? "https:" + game.cover.url.replace("t_thumb", "1080p")
                : "/default-cover.webp"
            }
          />
          <p className="font-semibold text-2xl">
            {reviewsAndRating?.avg._avg.rating}
          </p>
        </div>
      ...
      <div className="flex flex-col max-w-3/5 mx-auto p-2">
        ...
        {reviewsAndRating?.reviews.length ? (
          ...
          <p className="text-sm p-2">No reviews yet</p>
        )}
      </div>
    </div>
  );
